...
SELECT 
  _date,
  event_ts as bucketing_ts, 
  bucketing_id,
  experiment_id,
  associated_ids
from 
  `etsy-data-warehouse-prod.catapult_unified.filtering_event`
where 
  _date = current_date-1
)
, BucketingIndexesAndFilteringIndexe as(
  select distinct* from
  (select * from bucketing
  UNION ALL
  select * from filteringEvents)
)
, indexesOfInterest as (
  select b.*,e.bucketing_id_type as bucketing_id_type,
...
SELECT
  ...
  ARRAY_AGG(listing_id ORDER BY ABS(time_diff) LIMIT 1)[OFFSET(0)] AS segment_value
From
