...
SELECT
        ...
        event_id,
        event_type,
        min(filtered_bucketing_ts) AS filtered_bucketing_ts,
        sum(event_value) AS event_value,
        sum(filtered_event_value) AS filtered_event_value
...
CREATE OR REPLACE TABLE `etsy-data-warehouse-dev.madelinecollins.all_units_events_segments` AS (
    ...
    CROSS JOIN
        `etsy-data-warehouse-dev.madelinecollins.events`
    LEFT JOIN
        `etsy-data-warehouse-dev.madelinecollins.events_per_unit` epu
...
SELECT
    ...
    -- sum(case when event_id in ('total_winsorized_order_value') then event_count end)/100 ) end) AS acbv
