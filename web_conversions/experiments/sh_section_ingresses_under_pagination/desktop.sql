...
create or replace table etsy-data-warehouse-dev.madelinecollins.xp_section_ingress_desktop_channel_filtered as (
SELECT
  xp.variant_id,
  b.top_channel,
  COUNT(xp.bucketing_id) AS browsers,
...
/* following tables are for findings pvals in python */
-- Key Health Metrics (Winsorized ACBV and AOV) - Total (To compare with Catapult as a sanity check)
create or replace table etsy-data-warehouse-dev.madelinecollins.xp_section_ingress_desktop_python as (
SELECT
  xp.variant_id,
  xp.bucketing_id,
  -- metrics
...
-- Key Health Metrics (Winsorized ACBV and AOV) - Only browsers who viewed a listing page with review photos
create or replace table etsy-data-warehouse-dev.madelinecollins.xp_section_ingress_desktop_filtered_python as (
SELECT
  xp.variant_id,
  xp.bucketing_id,
  -- metrics
