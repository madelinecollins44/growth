...
select
  ...
  count(distinct case when shop_home_visit > 0 then browser_id end) as sh_browsers,
  count(distinct case when grid_visit > 0 then browser_id end) as grid_browsers,  
  count(distinct case when section_visit > 0 then browser_id end) as section_browsers,
from agg
group by all 
